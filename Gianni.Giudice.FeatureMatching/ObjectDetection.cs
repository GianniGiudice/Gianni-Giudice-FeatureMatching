...
namespace Gianni.Giudice.FeatureMatching;

public class ObjectDetection
{
    public async Task<List<ObjectDetectionResult>> DetectObjectInScenes(byte[] objectImageData, IList<byte[]> imagesSceneData)
    {
        ...
        foreach (var imageSceneData in imagesSceneData)
        {
            await Task.Run(() =>
            {
                list.Add(DetectObjectInScene(imageObjectData:objectImageData, imageSceneData));
            });
        }
